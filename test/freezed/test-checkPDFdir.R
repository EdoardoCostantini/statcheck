...
### Object:      Testing local checkPDFdir function against stable (CRAN) version
### Description: This script tests whether the extractions done using the stable 
###              version of checkPDFdir (on CRAN) and the version on which you are
###              working on (local) are giving the same results when applied to all
###              the PDF articles contained in a defined folder.
###              The criteria of comparison are: 1) number of extractions, 
###                                              2) number of errors, 
###                                              3) number of decision errors
### Requirements: (1) having a folder full of PDF articles;
...
context('PDF IMPORT: checkPDFdir output local = CRAN')
